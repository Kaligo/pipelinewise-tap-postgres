...
def do_sync_incremental(conn_config, stream, state, desired_columns, md_map):
    ...
    if stream_state:
        # Allowed bookmark keys for incremental sync
        # fast_sync_s3_info is allowed for backward compatibility when fast_sync_rds is disabled
        allowed_keys = {
            "replication_key",
            "replication_key_value",
            "version",
            "last_replication_method",
            "fast_sync_s3_info",  # Allowed for backward compatibility
        }
        illegal_bk_keys = set(stream_state.keys()).difference(allowed_keys)
...
def _create_fast_sync_rds_strategy(conn_config):
    ...
    if not s3_bucket:
        raise Exception(
            "fast_sync_rds_s3_bucket is required when fast_sync_rds is enabled"
        )

    s3_region = conn_config.get("fast_sync_rds_s3_region")
    if not s3_region:
        raise Exception(
            "fast_sync_rds_s3_region is required when fast_sync_rds is enabled"
        )
    ...
    fast_sync_rds_strategy = fast_sync_rds.FastSyncRdsStrategy(
        conn_config, s3_bucket, s3_prefix, s3_region
    )
...
def do_fast_sync_rds(conn_config, stream, state, desired_columns, md_map, sync_method):
    ...
    if fast_sync_rds_strategy is None:
        raise Exception(
            "fast_sync_rds_strategy is required for fast_sync_rds sync method"
        )
    ...
    if sync_method == "fast_sync_rds_full_table":
        state = fast_sync_rds_strategy.sync_table_full(
...
def sync_method_for_streams(
    streams, state, default_replication_method, fast_sync_rds_enabled=False
):
    """
    ...
    for stream in streams:
        ...
        if fast_sync_rds_enabled and replication_method in {
            "FULL_TABLE",
            "INCREMENTAL",
        }:
            lookup[stream["tap_stream_id"]] = (
                f"fast_sync_rds_{replication_method.lower()}"
            )
        ...
        if replication_method == "LOG_BASED" and stream_metadata.get((), {}).get(
            ...
            raise Exception(
                ...
                f"Please change the replication method for {stream_id}"
            )
...
def sync_traditional_stream(conn_config, stream, state, sync_method, end_lsn):
    ...
    if (
        sync_method == "fast_sync_rds_full_table"
        or sync_method == "fast_sync_rds_incremental"
    ):
        state = do_fast_sync_rds(
            conn_config, stream, state, desired_columns, md_map, sync_method
        )
        ...
        stream_id = stream["tap_stream_id"]
        raise Exception(f"unknown sync method {sync_method} for stream {stream_id}")
...
def do_sync(conn_config, catalog, default_replication_method, state, state_file=None):
    ...
    sync_method_lookup, traditional_streams, logical_streams = sync_method_for_streams(
        streams, state, default_replication_method, fast_sync_rds_enabled
    )
