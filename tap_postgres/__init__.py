...
def do_sync_incremental(conn_config, stream, state, desired_columns, md_map):
    ...
    illegal_bk_keys = set(stream_state.keys()).difference(
        {'replication_key', 'replication_key_value', 'version', 'last_replication_method', 'upper_bound', 'is_skip'})
...
def do_sync(conn_config, catalog, default_replication_method, state, state_file=None):
    ...
    for stream in traditional_streams:
        ...
        if conn_config["skip_streams_by_default"] and singer.get_bookmark(
            state, stream["tap_stream_id"], "is_skipped"
        ):
...
def main_impl():
    ...
    conn_config = {
        ...
        'skip_streams_by_default': args.config.get('skip_streams_by_default', False)
    }
